...
type
  ...
  public
    ...
    [Test]
    procedure TestColligoRange;
    [Test]
    procedure TestColligoRepeat;
    [Test]
    procedure TestColligoEmpty;
    [Test]
    procedure TestColligoGeneratorValidation;
    [Test]
...
begin
  FList := TColligoList<Integer>.Create;
end;
...
var
  LWords: IColligoList<string>;
  LGroups: IGroupByEnumerable<string, string>;
  LEnum: IColligoEnumerator<IGrouping<string, string>>;
  LCount: Integer;
begin
  // Case-insensitive comparer: 'a'/'A'/'a' collapse into one group.
  LWords := TColligoList<string>.Create;
...
var
  LGroups: IColligoList<IGrouping<Integer, Integer>>;
  LGroup: IGrouping<Integer, Integer>;
  LTotal: Integer;
begin
...
begin
  ...
  LJoined := FList.AsEnumerable.GroupJoin<string, Integer, string>(
    ...
    function(Num: Integer; Matches: IColligoEnumerableAdapter<string>): string
    begin
...
var
  LList2: IColligoList<string>;
  LZipped: IColligoEnumerable<string>;
  LArray: IColligoArray<string>;
begin
  FList.AddRange([1, 2, 3]);
  LList2 := TColligoList<string>.Create;
...
var
  LList: IColligoList<Variant>;
  LFiltered: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LList := TColligoList<Variant>.Create;
...
var
  LList: IColligoList<Variant>;
  LCast: IColligoEnumerable<Integer>;
  LRaised: Boolean;
begin
  LList := TColligoList<Variant>.Create;
...
var
  LList: IColligoList<Variant>;
  LEnum: IColligoEnumerator<Integer>;
  LRaised: Boolean;
begin
...
var
  LList: IColligoList<Double>;
  LSum: Double;
begin
  LList := TColligoList<Double>.Create;
...
var
  LSecond: IColligoList<Integer>;
  LExclude: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  FList.AddRange([1, 2, 3, 4]);
  LSecond := TColligoList<Integer>.Create;
...
var
  LSecond: IColligoList<Integer>;
  LExcluded: IColligoEnumerable<Integer>;
  LFirst, LSecondPass: IColligoArray<Integer>;
begin
...
var
  LFirst, LSecond: IColligoList<string>;
  LArray: IColligoArray<string>;
begin
  ...
  LFirst := TColligoList<string>.Create;
  LFirst.AddRange(['A', 'b', 'C']);
  LSecond := TColligoList<string>.Create;
...
var
  LCount: Integer;
  LDeferred: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
...
var
  LAppended, LPrepended: IColligoArray<Integer>;
begin
...
var
  LDeferred: IColligoEnumerable<Integer>;
  LFirst, LSecond: IColligoArray<Integer>;
begin
...
var
  LCount: Integer;
  LSecondKeys: IColligoList<Integer>;
  LDeferred: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  ...
  FList.AddRange([1, 1, 2, 3, 3, 4]);
  LSecondKeys := TColligoList<Integer>.Create;
...
var
  LReversed: IColligoEnumerable<Integer>;
  LFirst, LSecond: IColligoArray<Integer>;
begin
...
var
  LSource, LKeys: IColligoList<string>;
  LArray: IColligoArray<string>;
begin
  // Case-insensitive: key 'A' excludes 'a' -> ['B']. Default -> ['a','B'].
  LSource := TColligoList<string>.Create;
  LSource.AddRange(['a', 'B']);
  LKeys := TColligoList<string>.Create;
  LKeys.AddRange(['A']);
...
procedure TListTest.TestColligoRange;
var
  LArray: IColligoArray<Integer>;
begin
  // Range(1,5) = 1,2,3,4,5 (the 2nd arg is a COUNT, not an end value).
  LArray := TColligo.Range(1, 5).ToArray;
  Assert.AreEqual(5, LArray.Length, 'Range(1,5) has 5 elements');
  Assert.AreEqual(1, LArray[0], 'first');
  Assert.AreEqual(5, LArray[4], 'last');
  // Zero count -> empty.
  Assert.AreEqual(0, TColligo.Range(10, 0).ToArray.Length, 'Range(_,0) is empty');
  // Negative start supported.
  LArray := TColligo.Range(-2, 3).ToArray;
  ...
  LArray := TColligo.Range(High(Integer), 1).ToArray;
...
procedure TListTest.TestColligoRepeat;
var
  LArray: IColligoArray<string>;
begin
  LArray := TColligo.&Repeat<string>('x', 3).ToArray;
  ...
  Assert.AreEqual(0, TColligo.&Repeat<string>('x', 0).ToArray.Length, 'Repeat(_,0) is empty');
end;

procedure TListTest.TestColligoEmpty;
begin
  Assert.AreEqual(0, TColligo.Empty<Integer>.ToArray.Length, 'Empty<Integer> is empty');
  Assert.IsFalse(TColligo.Empty<string>.Any, 'Empty has no elements');
end;

procedure TListTest.TestColligoGeneratorValidation;
begin
...
var
  LCount: Integer;
  LChunkSource: IColligoEnumerableBase<TArray<Integer>>;
  LEnum: IColligoEnumerator<TArray<Integer>>;
...
var
  LSecond: TColligoList<Integer>;
  LIntersect: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  FList.AddRange([1, 2, 3, 4]);
  LSecond := TColligoList<Integer>.Create;
...
var
  LSecond: TColligoList<Integer>;
  LUnion: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  FList.AddRange([1, 2, 3]);
  LSecond := TColligoList<Integer>.Create;
...
var
  LSecond: TColligoList<Integer>;
  LConcat: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  FList.AddRange([1, 2, 3]);
  LSecond := TColligoList<Integer>.Create;
...
var
  LList: IColligoList<TArray<Integer>>;
  LSelected: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LList := TColligoList<TArray<Integer>>.Create;
  LList.Add([1, 2]);
  LList.Add([3]);
  LList.Add([4, 5]);
  LSelected := LList.AsEnumerable.SelectMany<Integer>(
    ...
    var
      LInnerList: TColligoList<Integer>;
    begin
      LInnerList := TColligoList<Integer>.Create(Value);
      try
...
var
  LList: IColligoList<string>;
  LFiltered: IColligoEnumerable<Char>;
  LResult: IColligoArray<Char>;
begin
  LList := TColligoList<string>.Create;
  LList.AddRange(['abc', 'def']);
  LFiltered := LList.AsEnumerable.SelectMany<Char>(
    function(x: string; i: integer): IColligoArray<Char>
    begin
      Result := TColligoArray<Char>.Create([x[1], x[1]]);
    end);
...
var
  LList: IColligoList<string>;
  LEnum: IColligoEnumerable<string>;
  LFiltered: IColligoEnumerable<Char>;
  LResult: IColligoArray<Char>;
begin
  LList := TColligoList<string>.Create(['abc', 'def']);
...
var
  LList: IColligoList<TOrderRec>;
  LArr: IColligoArray<TOrderRec>;
begin
  LList := TColligoList<TOrderRec>.Create;
...
var
  L1, L2: IColligoList<Integer>;
  LArr: IColligoArray<Integer>;
begin
  L1 := TColligoList<Integer>.Create;
  L1.AddRange([1, 1, 2, 3, 2]);
  L2 := TColligoList<Integer>.Create;
  L2.AddRange([1, 2]);
...
var
  LList: IColligoList<Integer>;
  LArr1, LArr2: IColligoArray<Integer>;
begin
  LList := TColligoList<Integer>.Create;
...
// TColligoList.From must not leak the wrapper (FastMM FullDebugMode fails on leak).
...
begin
  ...
  try
    LSrc.AddRange([1, 2, 3]);
    LArr := TColligoList<Integer>.From(LSrc).ToArray;
