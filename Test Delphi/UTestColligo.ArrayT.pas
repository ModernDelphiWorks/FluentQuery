...
begin
  FArray := TColligoArray<Integer>.Create([]);
end;
...
var
  LInner: IColligoArray<string>;
  LJoined: IColligoEnumerable<string>;
  LArray: IColligoArray<string>;
begin
  FArray.SetItems([1, 2, 3]);
  LInner := TColligoArray<string>.Create(['A1', 'B2', 'C3']);
...
var
  LArrayVar: IColligoArray<Variant>;
  LFiltered: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LArrayVar := TColligoArray<Variant>.Create([1, 'two', 3, 'four', 5]);
...
var
  LArray: TColligoArray<Double>;
  LSum: Double;
begin
  LArray := TColligoArray<Double>.Create([1.5, 2.5, 3.5]);
  try
...
begin
  FArray.SetItems([1, 2, 3]);
  LInner := TColligoArray<string>.Create(['A1', 'B1', 'C2']);
...
var
  LSecond: IColligoArray<Integer>;
  LExclude: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  FArray.SetItems([1, 2, 3, 4]);
  LSecond := TColligoArray<Integer>.Create([2, 4]);
...
begin
  FArray.SetItems([1, 2, 3, 4]);
  LSecond := TColligoArray<Integer>.Create([2, 4, 5]);
...
begin
  FArray.SetItems([1, 2, 3]);
  LSecond := TColligoArray<Integer>.Create([2, 3, 4]);
...
begin
  FArray.SetItems([1, 2, 3]);
  LSecond := TColligoArray<Integer>.Create([1, 2, 3]);
...
var
  LSecond: TColligoArray<Integer>;
  LResult: Boolean;
begin
  FArray.SetItems([1, 2, 3]);
  LSecond := TColligoArray<Integer>.Create([1, 2, 4]);
  try
    LResult := FArray.AsEnumerable.SequenceEqual(LSecond.AsEnumerable);
    Assert.IsFalse(LResult, 'Sequences should not be equal');
  finally
...
var
  LArrayStr: IColligoArray<string>;
  LFiltered: IColligoEnumerable<Char>;
  LResult: IColligoArray<Char>;
begin
  LArrayStr := TColligoArray<string>.Create(['abc', 'def']);
...
// TColligoArray.From must not leak the wrapper (FastMM FullDebugMode fails on leak).
procedure TArrayTest.TestArrayFromDoesNotLeak;
var
  LArr: IColligoArray<Integer>;
begin
  LArr := TColligoArray<Integer>.From(TArray<Integer>.Create(1, 2, 3)).ToArray;
