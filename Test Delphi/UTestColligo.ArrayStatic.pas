...
begin
  ...
  LArray[0] := LObj1;
  LArray[1] := LObj2;
  TColligoArray.FreeValues<TObject>(LArray);
...
var
  LEnum: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LEnum := TColligoArray.From<Integer>([1, 2, 3]);
...
begin
  ...
  TColligoArray.Sort<Integer>(LArray);
  Assert.AreEqual(1, LArray[0], 'First element should be 1');
...
begin
  ...
  TColligoArray.Sort<Integer>(LArray, LComparer);
  Assert.AreEqual(1, LArray[0], 'First element should be 1');
  Assert.AreEqual(5, LArray[4], 'Last element should be 5');
end;
...
begin
  ...
  LComparer := CreateComparer<Integer>;
  TColligoArray.Sort<Integer>(LArray, LComparer, 1, 4); // Sort from index 1 to 4
...
begin
  ...
  LComparer := CreateComparer<Integer>;
  Assert.IsTrue(TColligoArray.BinarySearch<Integer>(LArray, 3, LFoundIndex, LComparer, 0, 5), 'Should find 3');
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  LComparer := CreateComparer<Integer>;
  Assert.IsTrue(TColligoArray.BinarySearch<Integer>(LArray, 4, LFoundIndex, LComparer), 'Should find 4');
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  Assert.IsTrue(TColligoArray.BinarySearch<Integer>(LArray, 5, LFoundIndex), 'Should find 5');
...
begin
  // Demonstra uso com IColligoArray<T>, simplificando a atribuição, mas funciona.
  LSource := [1, 2, 3, 4, 5];
  LDest[0] := 0; LDest[1] := 0; LDest[2] := 0; LDest[3] := 0; LDest[4] := 0; LDest[5] := 0;
  TColligoArray.Copy<Integer>(LSource, LDest, 1, 2, 3);
...
begin
  ...
  TColligoArray.Copy<Integer>(LSource, LDest, 3);
...
var
  LResult: IColligoArray<Integer>;
begin
  LResult := TColligoArray.Concat<Integer>([TArray<Integer>.Create(1, 2),
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  Assert.AreEqual(NativeInt(2), TColligoArray.IndexOf<Integer>(LArray, 3), 'Index of 3 should be 2');
  Assert.AreEqual(NativeInt(-1), TColligoArray.IndexOf<Integer>(LArray, 6), 'Index of 6 should be -1');
end;
...
begin
  ...
  Assert.AreEqual(NativeInt(3), TColligoArray.IndexOf<Integer>(LArray, 2, 2), 'Index of 2 starting at 2 should be 3');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  LComparer := CreateComparer<Integer>;
  Assert.AreEqual(NativeInt(4), TColligoArray.IndexOf<Integer>(LArray, 5, LComparer, 0, 5), 'Index of 5 should be 4');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 2; LArray[4] := 5;
  Assert.AreEqual(NativeInt(3), TColligoArray.LastIndexOf<Integer>(LArray, 2), 'Last index of 2 should be 3');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 2; LArray[4] := 5;
  Assert.AreEqual(NativeInt(1), TColligoArray.LastIndexOf<Integer>(LArray, 2, 2), 'Last index of 2 up to index 2 should be 1');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 2; LArray[4] := 5;
  LComparer := CreateComparer<Integer>;
  Assert.AreEqual(NativeInt(3), TColligoArray.LastIndexOf<Integer>(LArray, 2, LComparer, 4, 5), 'Last index of 2 should be 3');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  Assert.IsTrue(TColligoArray.Contains<Integer>(LArray, 3), 'Should contain 3');
  Assert.IsFalse(TColligoArray.Contains<Integer>(LArray, 6), 'Should not contain 6');
end;
...
begin
  LArray[0] := 1; LArray[1] := 2; LArray[2] := 3; LArray[3] := 4; LArray[4] := 5;
  LComparer := CreateComparer<Integer>;
  Assert.IsTrue(TColligoArray.Contains<Integer>(LArray, 4, LComparer), 'Should contain 4');
end;
...
begin
  ...
  Assert.AreEqual('1,2,3', TColligoArray.ToString<Integer>(LArray), 'ToString should be "1,2,3"');
  Assert.AreEqual('[1] [2] [3]', TColligoArray.ToString<Integer>(LArray, ' ', '[', ']'), 'ToString with delimiters should be "[1] [2] [3]"');
end;
...
begin
  ...
  Assert.AreEqual('1,5;2,5;3,5', TColligoArray.ToString<Double>(LArray, LFmt, ';'), 'ToString with format should be "1,5;2,5;3,5"');
end;
...
var
  LArray: IColligoArray<Integer>;
begin
  LArray := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).ToArray;
...
var
  LResult: IColligoList<Integer>;
begin
  LResult := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).ToList;
...
var
  LCount: Integer;
begin
  LCount := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Count(
...
var
  LHasEven: Boolean;
begin
  LHasEven := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Any(
    function(Value: Integer): Boolean
    begin
...
var
  LFirstEven: Integer;
begin
  LFirstEven := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).FirstOrDefault(
    function(Value: Integer): Boolean
    begin
      Result := Value mod 2 = 0;
    end);
...
var
  LLastEven: Integer;
begin
  LLastEven := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).LastOrDefault(
    function(Value: Integer): Boolean
    begin
      Result := Value mod 2 = 0;
    end);
...
var
  LMin: Integer;
begin
  LMin := TColligoArray.From<Integer>([3, 1, 4, 1, 5]).Min;
...
var
  LMax: Integer;
begin
  LMax := TColligoArray.From<Integer>([3, 1, 4, 1, 5]).Max;
...
var
  LSum: Integer;
begin
  LSum := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Sum(
...
var
  LSum: Integer;
begin
  LSum := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Aggregate(
...
var
  LDistinct: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LDistinct := TColligoArray.From<Integer>([1, 2, 2, 3, 3, 4, 5, 5]).Distinct;
...
var
  LFiltered: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LFiltered := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Where(
    function(Value: Integer): Boolean
    begin
      Result := Value > 3;
    end);
...
var
  LTaken: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LTaken := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Take(3);
...
var
  LSkipped: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LSkipped := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Skip(2);
...
var
  LOrdered: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LOrdered := TColligoArray.From<Integer>([5, 2, 4, 1, 3]).OrderBy(
...
var
  LCount: Int64;
begin
  LCount := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).LongCount(
    function(Value: Integer): Boolean
    begin
      Result := Value > 2;
    end);
...
var
  LMapped: IColligoEnumerable<Integer>;
  LArray: IColligoArray<Integer>;
begin
  LMapped := TColligoArray.From<Integer>([1, 2, 3, 4, 5]).Select<Integer>(
    function(Value: Integer): Integer
    begin
      Result := Value * 2;
    end);
...
var
  ...
  LEnum: IColligoEnumerator<IGrouping<Integer, Integer>>;
...
var
  LZipped: IColligoEnumerable<string>;
  LArray: IColligoArray<string>;
begin
  LZipped := TColligoArray.From<Integer>([1, 2, 3]).Zip<string, string>(
    TColligoArray.From<string>(['A', 'B', 'C']),
...
var
  LJoined: IColligoEnumerable<string>;
  LArray: IColligoArray<string>;
begin
  LJoined := TColligoArray.From<Integer>([1, 2, 3]).Join<string, Integer, string>(
    TColligoArray.From<string>(['A1', 'B2', 'C3']),
...
var
  LMapped: IColligoEnumerable<string>;
  LArray: IColligoArray<string>;
begin
...
var
  ...
  LArray: IColligoArray<Integer>;
begin
  LOrdered := TColligoArray.From<Integer>([3, 1, 4, 1, 5]).Where(
    function(Value: Integer): Boolean
    begin
...
var
  ...
  LArray: IColligoArray<Integer>;
begin
  LDistinct := TColligoArray.From<Integer>([3, 1, 4, 1, 5, 3]).Where(
    function(Value: Integer): Boolean
    begin
      Writeln('Filtering: ' + IntToStr(Value));
      Result := Value > 2;
