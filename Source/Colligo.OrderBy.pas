...
type
  ...
  // first). Implements IColligoOrderedEnumerable<T> so ThenBy can append a
  ...
  TColligoOrderByEnumerable<T> = class(TColligoEnumerableBase<T>, IColligoOrderedEnumerable<T>)
  private
    FSource: IColligoEnumerableBase<T>;
    FComparers: TArray<TFunc<T, T, Integer>>;
    constructor CreateChain(const ASource: IColligoEnumerableBase<T>;
  ...
  TColligoOrderByEnumerator<T> = class(TInterfacedObject, IColligoEnumerator<T>)
  private type
    TIndexedItem = record
      Value: T;
      Index: Integer;
...
{ TColligoOrderByEnumerable<T> }

constructor TColligoOrderByEnumerable<T>.Create(const ASource: IColligoEnumerableBase<T>;
  const AComparer: TFunc<T, T, Integer>);
begin
  FSource := ASource;
...
constructor TColligoOrderByEnumerable<T>.CreateChain(const ASource: IColligoEnumerableBase<T>;
  const AComparers: TArray<TFunc<T, T, Integer>>);
begin
  FSource := ASource;
...
function TColligoOrderByEnumerable<T>.GetEnumerator: IColligoEnumerator<T>;
begin
  Result := TColligoOrderByEnumerator<T>.Create(FSource.GetEnumerator, FComparers);
end;

function TColligoOrderByEnumerable<T>.ThenByAppend(
  const AComparer: TFunc<T, T, Integer>): IColligoOrderedEnumerable<T>;
var
...
begin
  ...
  Result := TColligoOrderByEnumerable<T>.CreateChain(FSource, LNew);
end;

{ TColligoOrderByEnumerator<T> }

constructor TColligoOrderByEnumerator<T>.Create(const ASource: IColligoEnumerator<T>;
  const AComparers: TArray<TFunc<T, T, Integer>>);
var
...
function TColligoOrderByEnumerator<T>.GetCurrent: T;
begin
  Result := FItems[FIndex];
end;

function TColligoOrderByEnumerator<T>.MoveNext: Boolean;
begin
  Inc(FIndex);
  Result := FIndex < Length(FItems);
end;

procedure TColligoOrderByEnumerator<T>.Reset;
begin
  FIndex := -1;
end;
