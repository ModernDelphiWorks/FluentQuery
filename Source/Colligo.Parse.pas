...
type
  ...
  TColligoParseObjectDataSet<T> = class
  private
    function _ParseDataSet(const ADataSet: IDBDataSet): TColligoList<T>;
  public
    function ToList(const ADataSet: IDBDataSet): TColligoList<T>;
    function ToArray(const ADataSet: IDBDataSet): TColligoArray<T>;
    function ToDictionary<TKey, TValue>(
      const ADataSet: IDBDataSet;
      const AKeySelector: TFunc<T, TKey>;
      const AValueSelector: TFunc<T, TValue>): TColligoDictionary<TKey, TValue>;
  end;

  // Parser for scalar types (primitives or tuples)
  TColligoParseScalarDataSet<T> = class
  private
    function _ParseDataSet(const ADataSet: IDBDataSet): TColligoList<T>;
  public
    function ToList(const ADataSet: IDBDataSet): TColligoList<T>;
...
{ TColligoParseObjectDataSet<T> }

function TColligoParseObjectDataSet<T>._ParseDataSet(const ADataSet: IDBDataSet): TColligoList<T>;
var
  LItem: T;
  LValue: TValue;
  ...
  LFor: Integer;
begin
  Result := TColligoList<T>.Create;
  LContext := TRttiContext.Create;
  try
    LType := LContext.GetType(TypeInfo(T));
    if not (LType.TypeKind in [tkClass, tkInterface]) then
      raise Exception.Create('Type T must be a class for TColligoParseObjectDataSet');
...
function TColligoParseObjectDataSet<T>.ToList(const ADataSet: IDBDataSet): TColligoList<T>;
begin
  Result := _ParseDataSet(ADataSet);
end;

function TColligoParseObjectDataSet<T>.ToArray(const ADataSet: IDBDataSet): TColligoArray<T>;
var
  LList: IColligoList<T>;
begin
  LList := _ParseDataSet(ADataSet);
  try
    Result := TColligoArray<T>.Create(LList.ToArray.ArrayData);
  finally
    LList := nil;
  end;
end;

function TColligoParseObjectDataSet<T>.ToDictionary<TKey, TValue>(
  const ADataSet: IDBDataSet;
  const AKeySelector: TFunc<T, TKey>;
  const AValueSelector: TFunc<T, TValue>): TColligoDictionary<TKey, TValue>;
var
...
begin
  LList := _ParseDataSet(ADataSet);
  try
    Result := TColligoDictionary<TKey, TValue>.Create;
    try
...
{ TColligoParseScalarDataSet<T> }

function TColligoParseScalarDataSet<T>._ParseDataSet(const ADataSet: IDBDataSet): TColligoList<T>;
var
  LItem: T;
  LValue: TValue;
  LKeys: TArray<string>;
    ...
    try
      ...
      begin
        if (ADataSet.FieldCount = 1) and not
          ((LType.TypeKind = tkRecord) and (LType.Name = 'TColligoTuple<System.string>')) then
        begin
...
function TColligoParseScalarDataSet<T>.ToList(const ADataSet: IDBDataSet): TColligoList<T>;
begin
  Result := _ParseDataSet(ADataSet);
end;

function TColligoParseScalarDataSet<T>.ToArray(const ADataSet: IDBDataSet): TColligoArray<T>;
var
  LList: IColligoList<T>;
begin
  LList := _ParseDataSet(ADataSet);
...
function TColligoParseScalarDataSet<T>.ToDictionary<TKey, TValue>(
  const ADataSet: IDBDataSet;
  const AKeySelector: TFunc<T, TKey>;
  const AValueSelector: TFunc<T, TValue>): TColligoDictionary<TKey, TValue>;
var
