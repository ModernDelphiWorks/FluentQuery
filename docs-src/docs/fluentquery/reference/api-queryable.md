---
title: IFluentQueryable API
sidebar_position: 2
---

# IFluentQueryable&lt;T&gt; API reference

`IFluentQueryable<T>` is a value record in `FluentQuery.Queryable` that builds and executes SQL queries through a FluentSQL AST and a DataEngine connection.
...
|---|---|
| `Create(AQueryable: IFluentQueryableBase<T>)` | Wrap an existing queryable |
...
| `Select(AExpressions)` | `SELECT` | Array of `IFluentQueryExpression` |
| `Where(AExpression: string)` | `WHERE` | Raw SQL fragment |
| `Where(AExpression: array of const)` | `WHERE` | Variadic |
| `Where(AExpression: IFluentQueryExpression)` | `WHERE` | Type-safe expression |
...
| `Join<TInner, TResult>(AInner, AOuterKey, AInnerKey, AResultColumns)` | `INNER JOIN` | Returns `IFluentQueryable<TResult>` |
...
```delphi
var LQ: IFluentQueryable<TProduct>;
// ...
...
| `AsEnumerable` | `IFluentEnumerable<T>` | Convert to in-memory pipeline |

## IFluentQueryExpression operators

`IFluentQueryExpression` (unit `FluentQuery.Expression`) constructs typed SQL predicates:
