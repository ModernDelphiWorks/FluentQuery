...
# Collections API — TFluentList, TFluentArray, TFluentDictionary

FluentQuery ships three concrete collection classes in `FluentQuery.Collections` that implement their interface counterparts and integrate natively with `IFluentEnumerable<T>`.
...
When `AOwnerships = True` and `T` is a class, `TFluentList<T>` frees owned objects on removal/destruction.
...
```delphi
class function From(const AList: TList<T>): IFluentEnumerable<T>;
class function From(const AArray: TArray<T>): IFluentEnumerable<T>;
```
...
```delphi
class function From(const AArray: TArray<T>): IFluentEnumerable<T>; overload;
class function From(const AList: TList<T>): IFluentEnumerable<T>; overload;
```

## TFluentArray (non-generic utility record)
...
class function From<T>(const AArray: array of T): IFluentEnumerable<T>;
```
