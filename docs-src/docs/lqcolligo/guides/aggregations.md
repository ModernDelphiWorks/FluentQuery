...
```delphi
var LAll := TLQColligoArray<Integer>.From([1, 2, 3, 4, 5]).Count; // 5

var LEven := TLQColligoArray<Integer>.From([1, 2, 3, 4, 5])
  .Count(function(const X: Integer): Boolean begin Result := X mod 2 = 0 end);
// 2

// LongCount returns Int64:
var LLong := TLQColligoArray<Integer>.From([1, 2, 3]).LongCount;
```

## Sum

`Sum` accepts a typed selector. Overloads for `Double`, `Integer`, `Int64`, `Single`, `Currency`, and `LQColligoNullable<*>` variants:

```delphi
var LTotal := TLQColligoArray<Double>.From([1.0, 2.0, 3.0])
  .Sum(function(const X: Double): Double begin Result := X end);
// LTotal = 6.0

var LIntSum := TLQColligoArray<Integer>.From([1, 2, 3])
...
```delphi
var LAvg := TLQColligoArray<Integer>.From([1, 2, 3, 4, 5])
...
```delphi
var LMin := TLQColligoArray<Integer>.From([3, 1, 4]).Min; // 1
var LMax := TLQColligoArray<Integer>.From([3, 1, 4]).Max; // 4
```

Typed-selector overloads return the extreme value of the extracted key:

```delphi
var LLowest := TLQColligoArray<Double>.From([2.5, 1.1, 3.3])
...
```delphi
var LFirst := TLQColligoArray<Integer>.From([5, 3, 1]).First; // 5

var LMatch := TLQColligoArray<Integer>.From([1, 2, 3, 4])
...
```delphi
var LProduct := TLQColligoArray<Integer>.From([1, 2, 3, 4])
...
```delphi
var LSum := TLQColligoArray<Integer>.From([1, 2, 3])
  .Aggregate<Integer>(0,
...
```delphi
var LSet := TLQColligoArray<Integer>.From([1, 2, 2, 3]).ToHashSet;
// THashSet<Integer> with {1, 2, 3}

var LDict := TLQColligoArray<TUser>.From(LUsers)
