...
- `Items` — an `ILQColligoEnumerable<T>` of the group's elements.
...
Projects each group directly to a result value. The `AResultSelector` receives the group key and an `ILQColligoEnumerableAdapter<T>` wrapping the group's elements (call `.AsEnumerable` to iterate them):
...
var LResult := TLQColligoArray<TProduct>.From(LProducts)
  .GroupBy<string, TSummary>(
    ...
    function(const Key: string; const Group: ILQColligoEnumerableAdapter<TProduct>): TSummary
    var
      LSum: Double;
      LItem: TProduct;
    begin
...
```delphi
var LEnumerable := TLQColligoArray<TProduct>.From(LProducts)
  .GroupBy<string>(function(const P: TProduct): string begin Result := P.Category end)
  .AsEnumerable;
// ILQColligoEnumerable<IGrouping<string, TProduct>>
```
