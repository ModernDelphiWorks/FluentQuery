...
```delphi
// function TakeWhile(const APredicate: TFunc<T, Integer, Boolean>): ILQColligoEnumerable<T>;
```
...
```delphi
// function SkipWhile(const APredicate: TFunc<T, Integer, Boolean>): ILQColligoEnumerable<T>;
```
...
```delphi
var LValue := TLQColligoArray<string>.From(['a', 'b', 'c']).ElementAt(1);
// LValue = 'b'

var LSafe := TLQColligoArray<string>.From(['a', 'b']).ElementAtOrDefault(5);
...
begin
  LPage := TLQColligoArray<TRecord>.From(AAll)
    .Skip(APage * PageSize)
    .Take(PageSize)
    .ToArray;
end;
