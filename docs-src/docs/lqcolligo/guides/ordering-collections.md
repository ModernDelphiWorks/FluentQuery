...
LQ-Colligo provides multiple ordering operators, all of which buffer the full sequence before returning the first element.
...
```delphi
var LResult := TLQColligoArray<string>.From(['banana', 'apple', 'cherry'])
...
```delphi
var LResult := TLQColligoArray<Integer>.From([3, 1, 4, 1, 5])
...
```delphi
var LResult := TLQColligoArray<Integer>.From([3, 1, 2])
  .Order
  .ToArray;
// Result: [1, 2, 3]

var LDesc := TLQColligoArray<Integer>.From([3, 1, 2])
  .OrderDescending
  .ToArray;
// Result: [3, 2, 1]
```
...
```delphi
var LResult := TLQColligoArray<string>.From(['b', 'A', 'c'])
...
var LResult := TLQColligoArray<TEmployee>.From(LEmployees)
