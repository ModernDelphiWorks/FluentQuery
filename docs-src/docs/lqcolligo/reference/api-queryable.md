---
title: ILQColligoQueryable API
sidebar_position: 2
---

# ILQColligoQueryable&lt;T&gt; API reference

`ILQColligoQueryable<T>` is a value record in `LQColligo.Queryable` that builds and executes SQL queries through a FluentSQL AST and a DataEngine connection.
...
|---|---|
| `Create(AQueryable: ILQColligoQueryableBase<T>)` | Wrap an existing queryable |
...
| `Select(AExpressions)` | `SELECT` | Array of `ILQColligoQueryExpression` |
| `Where(AExpression: string)` | `WHERE` | Raw SQL fragment |
| `Where(AExpression: array of const)` | `WHERE` | Variadic |
| `Where(AExpression: ILQColligoQueryExpression)` | `WHERE` | Type-safe expression |
...
| `Join<TInner, TResult>(AInner, AOuterKey, AInnerKey, AResultColumns)` | `INNER JOIN` | Returns `ILQColligoQueryable<TResult>` |
...
```delphi
var LQ: ILQColligoQueryable<TProduct>;
// ...
...
| `AsEnumerable` | `ILQColligoEnumerable<T>` | Convert to in-memory pipeline |

## ILQColligoQueryExpression operators

`ILQColligoQueryExpression` (unit `LQColligo.Expression`) constructs typed SQL predicates:
